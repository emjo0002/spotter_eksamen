...
export default function StickyMascot() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40 flex flex-row items-end gap-2">
      {/* Taleboble */}
      {showBubble && (
        <div ref={bubbleRef} style={{ opacity: 0 }} className="bg-[var(--white)] text-[var(--black)] px-4 py-2 rounded-lg shadow-lg text-sm relative">
      ...
      {/* Chatboks */}
      {chatOpen && (
        <div ref={chatRef} style={{ opacity: 0 }} className="bg-[var(--white)] text-[var(--black)] w-64 p-4 rounded-lg shadow-xl text-sm space-y-2 flex flex-col relative">
      ...
      <button onClick={toggleChat} className="w-16 h-16 overflow-hidden">
        <Image src="/photos/spottermaskot.png" alt="Sticky mascot" width={64} height={64} className="object-cover w-full h-full" />
      </button>
    </div>
  );
}
